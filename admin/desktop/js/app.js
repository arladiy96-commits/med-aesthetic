...
(() => {
  ...
  const ADMIN_APP = true;
  const startupRole = entryHint === "creator" ? "creator" : "admin";
  const startupSwitch = startupRole === "creator" ? ["creator","client"] : ["admin"];
  ...
  async function loadIdentity(){
    ...
    try{
      ...
      if(actualRole!=="creator" && actualRole!=="admin"){
        location.replace("/client/index.html?entry=client");
        return;
      }

      if(actualRole==="creator" && keepCreatorClientPreview){
        S.roleMode="client";
        if(String(S.view).startsWith("admin-") || S.view==="role-loading") S.view="home";
      }else if(actualRole==="creator"){
        S.roleMode="creator";
        if(!String(S.view).startsWith("admin-")) S.view="admin-home";
      }else if(actualRole==="admin"){
        // Администратор всегда администратор, пока создатель не снимет права.
        S.roleMode="admin";
        if(!String(S.view).startsWith("admin-")) S.view="admin-home";
      }else{
        S.roleMode="client";
  ...
  function adminIsDesktopLayout(){ return true; }

  function syncAdminLayoutClass(){
    document.body?.classList.remove("client-layout","admin-layout-mobile","admin-layout-desktop","admin-mobile-light");
    document.body?.classList.add("admin-layout-desktop");
    return true;
  }
  ...
  window.Beauty = {
    ...
    switchRole(role){
      if(S.actualRole==="admin")return;
      if(!S.roleSwitch.includes(role))return;
      if(role==="client"){
        location.href="/client/index.html?entry=client&preview=1";
        return;
      }
      S.roleMode=S.actualRole;
      S.roleSheet=false;
      S.view="admin-home";
      render();
      loadAdminBookings(true);
